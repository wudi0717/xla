...
/* static */ void GlobalData::Release(
  ...
  absl::flat_hash_map<Service*, std::vector<GlobalDataHandle>> parent_handles;
  for (auto& instance : instances) {
    if (instance->parent_ != nullptr) {
      parent_handles[instance->parent_].push_back(instance->Release());
    }
  }
  for (auto& [parent, handles] : parent_handles) {
    ReleaseHandles(parent, handles);
  }
}
...
StatusOr<ChannelHandle> Service::CreateChannelHandle(
    ChannelHandle::ChannelType type) {
  CreateChannelHandleRequest request;
  request.set_channel_type(type);
  CreateChannelHandleResponse response;
  TF_RETURN_IF_ERROR(CreateChannelHandle(&request, &response));
  return response.channel();
}

Status Service::Unregister(const GlobalDataHandle& data) {
  UnregisterRequest request;
  *request.add_data() = data;
  UnregisterResponse response;
  return Unregister(&request, &response);
}

StatusOr<std::vector<std::unique_ptr<GlobalData>>> Service::DeconstructTuple(
    const GlobalData& data) {
  DeconstructTupleRequest request;
  *request.mutable_tuple_handle() = data.handle();
  DeconstructTupleResponse response;
  TF_RETURN_IF_ERROR(DeconstructTuple(&request, &response));
  std::vector<std::unique_ptr<GlobalData>> handles;
  for (auto& handle : response.element_handles()) {
    handles.push_back(std::make_unique<GlobalData>(this, handle));
  }
  return handles;
}

StatusOr<ExecutionHandle> Service::Compile(
  ...
  *request.mutable_execution_options() = execution_options;
  ...
  TF_RETURN_IF_ERROR(Compile(&request, &response));
  if (!response.has_handle()) {
    return FailedPrecondition("service compile returned no executable handle");
  }
  return response.handle();
}

StatusOr<std::unique_ptr<GlobalData>> Service::Execute(
  ...
  *request.mutable_handle() = handle;
  ...
  TF_RETURN_IF_ERROR(Execute(&request, &response));
  if (execution_profile != nullptr) {
    *execution_profile = response.profile();
  }
  if (!response.has_output()) {
    return FailedPrecondition("service execute returned no output handle");
  }
  return std::make_unique<GlobalData>(this, response.output());
}

StatusOr<std::vector<std::unique_ptr<GlobalData>>> Service::ExecuteGraphParallel(
  ...
  TF_RETURN_IF_ERROR(ExecuteGraphParallel(&request, &response));

  std::vector<std::unique_ptr<GlobalData>> outputs;
  outputs.reserve(response.responses_size());
  for (size_t i = 0; i < response.responses_size(); ++i) {
    outputs.push_back(
        std::make_unique<GlobalData>(this, response.responses(i).output()));
    if (i < computations.size() && computations[i].execution_profile != nullptr) {
...
StatusOr<std::vector<DeviceHandle>> Service::GetDeviceHandles(
  ...
  TF_RETURN_IF_ERROR(GetDeviceHandles(&request, &response));
  std::vector<DeviceHandle> handles;
  handles.reserve(response.device_handles_size());
  for (const DeviceHandle& handle : response.device_handles()) {
    handles.push_back(handle);
  }
  return handles;
}

StatusOr<Literal> Service::TransferToClient(const GlobalData& data,
  ...
  *request.mutable_data() = data.handle();
  ...
  TF_RETURN_IF_ERROR(TransferToClient(&request, &response));
  if (!response.has_literal()) {
    return FailedPrecondition(
        "service TransferToClient returned no literal payload");
  }
  return Literal::CreateFromProto(response.literal());
}

StatusOr<std::unique_ptr<GlobalData>> Service::TransferToServer(
  ...
  *request.mutable_literal() = literal.ToProto();
  ...
  TF_RETURN_IF_ERROR(TransferToServer(&request, &response));
  if (!response.has_data()) {
    return FailedPrecondition(
        "service TransferToServer returned no global data handle");
  }
  return std::make_unique<GlobalData>(this, response.data());
}

Status Service::TransferToInfeed(const LiteralSlice& literal, int64_t replica_id,
  ...
  *request.mutable_literal() = literal.ToProto();
  ...
  return TransferToInfeed(&request, &response);
}

StatusOr<Literal> Service::TransferFromOutfeed(
  ...
  TF_RETURN_IF_ERROR(TransferFromOutfeed(&request, &response));
  if (!response.has_literal()) {
    return FailedPrecondition(
        "service TransferFromOutfeed returned no literal payload");
  }
  return Literal::CreateFromProto(response.literal());
}

Status Service::ResetDevice() {
  ResetDeviceRequest request;
  ResetDeviceResponse response;
  return ResetDevice(&request, &response);
}

StatusOr<Literal> Service::ComputeConstantGraph(
  ...
  *request.mutable_computation() = computation.proto();
  ...
  TF_RETURN_IF_ERROR(ComputeConstantGraph(&request, &response));
  if (!response.has_literal()) {
    return InternalError(
        "service ComputeConstantGraph returned no literal payload");
  }
  return Literal::CreateFromProto(response.literal());
}

StatusOr<Shape> Service::GetShape(const GlobalData& data) {
  GetShapeRequest request;
  *request.mutable_data() = data.handle();
  GetShapeResponse response;
  TF_RETURN_IF_ERROR(GetShape(&request, &response));
