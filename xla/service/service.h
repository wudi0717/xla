...
class Service : public ServiceInterface {
 public:
  ...
  StatusOr<Literal> TransferToClient(const GlobalData& data,
                                     const Shape* shape_with_layout);
  StatusOr<std::unique_ptr<GlobalData>> TransferToServer(
      const LiteralSlice& literal, const DeviceHandle* device_handle);
  Status TransferToInfeed(const LiteralSlice& literal, int64_t replica_id,
                          const DeviceHandle* device_handle);
  StatusOr<Literal> TransferFromOutfeed(const Shape* shape_with_layout,
                                        int64_t replica_id,
                                        const DeviceHandle* device_handle);
  Status ResetDevice();
  StatusOr<Literal> ComputeConstantGraph(const XlaComputation& computation,
                                         const Layout* output_layout);
  StatusOr<Shape> GetShape(const GlobalData& data);
  StatusOr<ChannelHandle> CreateChannelHandle(ChannelHandle::ChannelType type);
