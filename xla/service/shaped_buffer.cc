...
void ScopedShapedBuffer::Deallocate() {
  ...
  if (IsMusaDebugDeallocEnabled()) {
    LOG(INFO) << "[MUSA_DEALLOC_DEBUG] ScopedShapedBuffer::Deallocate begin "
  ...
  for (auto& pair : buffers_) {
    ...
    if (!memory_base.is_null() &&
      ...
      if (IsMusaDebugDeallocEnabled()) {
        LOG(INFO) << "[MUSA_DEALLOC_DEBUG] ScopedShapedBuffer leaf begin "
                  << "index=" << index.ToString() << " ptr="
                  << memory_base.opaque() << " size=" << memory_base.size()
                  << "B";
      }
      TF_CHECK_OK(allocator_->Deallocate(device_ordinal(), memory_base));
      if (IsMusaDebugDeallocEnabled()) {
        LOG(INFO) << "[MUSA_DEALLOC_DEBUG] ScopedShapedBuffer leaf end "
                  << "index=" << index.ToString();
      }
    }
  }
  if (IsMusaDebugDeallocEnabled()) {
    LOG(INFO) << "[MUSA_DEALLOC_DEBUG] ScopedShapedBuffer::Deallocate end "
              << "device_ordinal=" << device_ordinal();
  }
}
