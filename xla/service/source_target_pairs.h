...
class SourceTargetPairs {
 ...
 public:
  ...
  static StatusOr<SourceTargetPairs> FromString(absl::string_view str) {
  ...
  template <typename H>
  friend H AbslHashValue(H h, const SourceTargetPair& pair) {
    return H::combine(std::move(h), pair.source, pair.target);
  }
