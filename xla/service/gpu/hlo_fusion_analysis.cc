...
const TilingScheme* HloFusionAnalysis::GetTransposeTilingScheme() {
  ...
  const int64_t warp_size = WarpSize(*device_info_);
...
bool HloFusionAnalysis::IsDeviceAwareUnnestedReduction(
    const HloInstruction& root) const {
  CHECK(device_info_ != nullptr);
  return IsReductionFromOrToContiguousDimensions(root, *device_info_);
}
...
int64_t HloFusionAnalysis::MaxBeneficialColumnReductionUnrollBasedOnBlockSize()
  ...
  for (const HloInstruction* root : fusion_roots()) {
    if (!IsDeviceAwareUnnestedReduction(*root)) {
      continue;
    }
...
bool HloFusionAnalysis::IsUnrollingColumnReductionBeneficial(
  ...
  const int64_t warp_size = WarpSize(*device_info_);
  if (num_kept_minor % (warp_size * 2) != 0) {
    return false;
  }
  ...
  for (const HloInstruction* fusion_root : fusion_roots()) {
    if (!reduction_is_race_free &&
        IsDeviceAwareUnnestedReduction(*fusion_root)) {
...
bool HloFusionAnalysis::CanVectorizeReduction(
  ...
  const int64_t warp_size = WarpSize(*device_info_);
  if (reduction_dimensions.is_row_reduction && num_threads_x <= warp_size) {
    return false;
  }
...
int HloFusionAnalysis::CalculateVirtualThreadScalingFactorForReduction(
  ...
  if (reduction_dimensions.is_row_reduction) {
    const int64_t warp_size = WarpSize(*device_info_);
    return RowReductionGetRowsPerWarp(reduction_dimensions.dimensions[kDimX],
                                      warp_size);
  }
  return 1;
}
...
ReductionCodegenInfo HloFusionAnalysis::ComputeReductionCodegenInfo(
  ...
  int64_t num_threads_y =
      reduction_dimensions.is_row_reduction ? 1 : warp_size;
  int64_t num_threads_x = [&] {
    if (reduction_dimensions.is_row_reduction) {
      if (RowReductionGetRowsPerWarp(reduction_dimensions.dimensions[2],
                                     warp_size) > 1) {
