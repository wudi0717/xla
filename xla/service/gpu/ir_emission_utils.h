...
inline int64_t WarpSize(const se::DeviceDescription& device_info) {
  return device_info.threads_per_warp() > 0 ? device_info.threads_per_warp()
                                            : WarpSize();
}
...
// aren't 32 bits in size. The last operand of the emitted "shfl" is the
// target warp width minus one.
//
