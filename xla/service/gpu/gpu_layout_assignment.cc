...
bool GpuLayoutAssignment::PropagateReductionLayoutToOperand(
  ...
  // We try to propagate a layout that makes the reduction a row reduction.
  // This is only beneficial when the tiled reduction emitter is expected to
  // beat the elemental emitter on the current device.
