...
static Status LowerToXlaGpuRuntime(
  ...
  if (target_triple == mtgpu::TargetTriple()) {
    // MTGPU runtime path does not register graph launch custom calls.
    // Force-disable graph outlining so lowering never emits graph symbols.
    opts.gpu_graph_level = 0;
  }
...
StatusOr<GpuExecutable::OwnedGpuRuntimeProgram> LowerToJitRt(
  ...
  TF_RETURN_IF_ERROR(LowerToXlaGpuRuntime(
      ...
      compute_capability, target_triple));
...
Status CompileModuleToLlvmIrImpl(
...

  {
    ...
    bool supports_runtime_managed_constants =
        ...
        target_triple != mtgpu::TargetTriple() &&
  ...
  if (IsXlaRuntimeExecutableEnabled(hlo_module->config()) &&
    ...
    TF_ASSIGN_OR_RETURN(
        ...
        LowerToJitRt(*mlir_module, entry_function.getName(), buffer_sizes,
                     ...
                     gpu_device_info.gpu_compute_capability(), target_triple));
    return OkStatus();
  }
