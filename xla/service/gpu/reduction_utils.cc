...
int64_t ReductionDimensionRaceFreeBound(
  ...
  if (reduction_dimensions.is_row_reduction) {
    return 1024 * reduction_tiling[2];
  }
  const int64_t warp_size =
      device_description.threads_per_warp() > 0
          ? device_description.threads_per_warp()
          : 128;
  return warp_size * reduction_tiling[1];
}

bool IsUnnestedReductionFasterThanElemental(
    const ReductionDimensions& reduction_dimensions) {
  // Legacy callers of this overload do not thread device_description through
  // yet. Use the current MTGPU warp size so the old reduction heuristics stay
  // aligned with the backend we are bringing up.
  const int64_t warp_size = 128;
  if (reduction_dimensions.is_row_reduction) {
...
bool ReductionIsRaceFree(const HloModuleConfig& hlo_module_config,
  ...
  return reduction_dimensions.dimensions[1] <= 128 * reduction_tiling[1];
}
