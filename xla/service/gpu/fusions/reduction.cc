...
ReductionCodegenState GenerateReductionCodegenState(
    ...
    FusedIrEmitter& fused_emitter, int64_t warp_size) {
    ...
    for (int op_result_idx = 0;
      ...
      llvm::GlobalVariable* shared_cache = [&]() -> llvm::GlobalVariable* {
        if (reduction_codegen_state.IsRowReduction()) {
          ...
          CHECK_EQ(tiling_scheme.GetNumThreadsPerBlock() % warp_size, 0);
          int num_warps = tiling_scheme.GetNumThreadsPerBlock() / warp_size;
...
void EmitFullWarpShuffleDownLoopForReduce(
    ...
    int threads_per_block, int num_results_per_warp, int64_t warp_size) {
  // This only works when the block size is a multiple of the device warp size.

  // We check this here as a mistake in the number of threads per
  // block is very hard to detect.
  CHECK_GT(warp_size, 0);
  CHECK_EQ(threads_per_block % warp_size, 0);
  CHECK_EQ(warp_size % num_results_per_warp, 0);

  for (int distance = warp_size / (2 * num_results_per_warp); distance >= 1;
       distance /= 2) {
...
void EmitReductionOutputForRowReduction(
  ...
  int num_rows_per_warp =
      RowReductionGetRowsPerWarp(reduced_dimension_size, warp_size);
  ...
  llvm::Value* warp_id =
      builder->CreateUDiv(thread_id_info.thread_id_x, constant(warp_size));
  ...
  ksl.If("inter_warp_reduce", is_zero(warp_id), [&] {
    ...
    if (tiling_scheme.GetNumThreadsPerBlock() > warp_size) {
      EmitFullWarpShuffleDownLoopForReduce(
...
void EmitReductionOutputForColumnReduction(
  ...
  EmitFullWarpShuffleDownLoopForReduce(builder, ir_emitter_context, reducer,
                                       ...
                                       /*num_results_per_warp=*/1, warp_size);
...
Status EmitIRForReduction(
  ...
  int64_t warp_size = WarpSize(ir_emitter_context.gpu_device_info());
  CHECK_EQ(tiling_scheme.GetNumThreadsPerBlockPhysical() % warp_size, 0);
  llvm::Type* index_ty =
  ...
  ReductionCodegenState codegen_state = GenerateReductionCodegenState(
      builder, fusion, reduction_info, heroes, fused_emitter, warp_size);
  ...
  if (!reduction_codegen_info->IsRaceFree()) {
    ...
    for (int i = 0; i < fusion_roots.size(); ++i) {
      if (IsReductionFromOrToContiguousDimensions(
              *fusion_roots[i], ir_emitter_context.gpu_device_info())) {
        TF_ASSIGN_OR_RETURN(
