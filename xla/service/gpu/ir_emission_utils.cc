...
//
// MTGPU keeps using the NVVM shfl emission path here; the backend later rewrites
// the intrinsic name to the corresponding MUSA intrinsic. The width/mask
// operand therefore must already match the actual device warp size.
llvm::Value* EmitNVPTXShflDown(llvm::Value* value, llvm::Value* offset,
                               llvm::IRBuilder<>* b, int64_t warp_size) {
  llvm::Module* module = b->GetInsertBlock()->getModule();
  ...
  return b->CreateCall(
      intrinsic,
      {b->getInt32(-1), value, offset,
       b->getInt32(static_cast<int32_t>(warp_size - 1))});
}
...
llvm::Value* EmitFullWarpShuffleDown(llvm::Value* value, llvm::Value* offset,
  ...
  if (value->getType()->isFloatTy() && bit_width == 32) {
    if (target_triple.isNVPTX() ||
        target_triple.str() == mtgpu::TargetTriple()) {
      return EmitNVPTXShflDown(value, offset, builder, warp_size);
  ...
  for (int i = 0; i < num_segments; ++i) {
    ...
    if (target_triple.isNVPTX() ||
        target_triple.str() == mtgpu::TargetTriple()) {
      insert_val = EmitNVPTXShflDown(builder->CreateExtractElement(x, i), offset,
                                     builder, warp_size);
    } else if (target_triple.getArch() == llvm::Triple::amdgcn) {
