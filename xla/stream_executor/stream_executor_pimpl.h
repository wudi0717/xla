...
class StreamExecutor {
 public:
  ...
  // Creates and initializes a StreamCommon instance using the existing
  // AllocateStream/DeallocateStream lifecycle.
  tsl::StatusOr<std::unique_ptr<Stream>> CreateStream(
      std::optional<std::variant<StreamPriority, int>> priority);
  tsl::StatusOr<std::unique_ptr<Stream>> CreateStream() {
    return CreateStream(std::nullopt);
  }
  tsl::StatusOr<std::unique_ptr<Stream>> CreateStream(StreamPriority priority) {
    return CreateStream(std::variant<StreamPriority, int>(priority));
  }
  tsl::StatusOr<std::unique_ptr<Stream>> CreateStream(int priority) {
    return CreateStream(std::variant<StreamPriority, int>(priority));
  }
