...
const std::string& Stream::GetName() const { return name_; }
...
tsl::Status Stream::RecordEvent(Event* event) {
...
Stream &Stream::ThenRecordEvent(Event *event) {
  ...
  tsl::Status status = RecordEvent(event);
  if (!status.ok()) {
...
tsl::StatusOr<Stream*> Stream::GetOrCreateSubStream() {
...
tsl::Status Stream::WaitFor(Stream* other) {
  return parent_->CreateStreamDependency(this, other)
             ? ::tsl::OkStatus()
             : tsl::errors::Internal("failed to create stream dependency");
}
...
tsl::Status Stream::WaitFor(Event* event) {
...
Stream &Stream::ThenWaitFor(Event *event) {
  ...
  if (ok()) {
    tsl::Status status = WaitFor(event);
    if (!status.ok()) {
...
tsl::Status Stream::Memcpy(void* host_dst, const DeviceMemoryBase& gpu_src,
                           uint64_t size) {
  return parent_->Memcpy(this, host_dst, gpu_src, size)
             ? ::tsl::OkStatus()
             : tsl::errors::Internal("failed to enqueue memcpy from device");
}
...
tsl::Status Stream::Memcpy(DeviceMemoryBase* gpu_dst, const void* host_src,
                           uint64_t size) {
  return parent_->Memcpy(this, gpu_dst, host_src, size)
             ? ::tsl::OkStatus()
             : tsl::errors::Internal("failed to enqueue memcpy to device");
}
...
tsl::Status Stream::Memcpy(DeviceMemoryBase* gpu_dst,
                           const DeviceMemoryBase& gpu_src, uint64_t size) {
  return parent_->MemcpyDeviceToDevice(this, gpu_dst, gpu_src, size)
             ? ::tsl::OkStatus()
             : tsl::errors::Internal("failed to enqueue device memcpy");
}
...
tsl::Status Stream::MemZero(DeviceMemoryBase* location, uint64_t size) {
...
tsl::Status Stream::Memset32(DeviceMemoryBase* location, uint32_t pattern,
                             uint64_t size) {
...
tsl::Status Stream::DoHostCallbackWithStatus(
    absl::AnyInvocable<tsl::Status() &&> callback) {
  if (!ok()) {
    LOG(INFO) << DebugStreamPointers()
              << " was in error state before adding host callback";
  }
  return parent_->HostCallback(this, std::move(callback))
             ? ::tsl::OkStatus()
             : tsl::errors::Internal("failed to enqueue host callback");
}
