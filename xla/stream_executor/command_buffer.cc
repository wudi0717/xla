...
/*static*/ tsl::StatusOr<CommandBuffer> CommandBuffer::Trace(
    ...
    Mode mode) {
  // Route tracing through the central executor stream factory so command buffer
  // setup follows the same lifecycle as other common stream users.
  TF_ASSIGN_OR_RETURN(auto stream, executor->CreateStream());
  Stream* trace_stream = stream.get();
  ...
  TF_RETURN_IF_ERROR(command_buffer.implementation()->Trace(
      trace_stream, [&]() { return function(trace_stream); }));
