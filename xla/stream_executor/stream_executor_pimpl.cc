...
tsl::StatusOr<std::unique_ptr<Stream>> StreamExecutor::CreateStream(
    std::optional<std::variant<StreamPriority, int>> priority) {
  std::unique_ptr<Stream> custom_stream;
  tsl::Status custom_status =
      implementation_->CreateCustomStream(this, priority, &custom_stream);
  if (custom_status.ok()) {
    return std::move(custom_stream);
  }
  if (custom_status.code() != tsl::error::UNIMPLEMENTED) {
    return custom_status;
  }

  auto stream = std::make_unique<StreamCommon>(this, priority);
  if (priority.has_value()) {
    std::visit(
        [&](auto stream_priority) { stream->SetPriority(stream_priority); },
        *priority);
  }
  stream->Init();
  if (!stream->ok()) {
    return tsl::errors::Internal("failed to initialize stream");
  }
  return stream;
}
