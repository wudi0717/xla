...
class Stream {
 public:
  ...
  // stream.
  virtual bool ok() const { return !InErrorState(); }
  ...
  // execution status.
  virtual tsl::Status RefreshStatus() TF_LOCKS_EXCLUDED(mu_);
  ...
  virtual tsl::StatusOr<Stream *> GetOrCreateSubStream() TF_LOCKS_EXCLUDED(mu_);

  // Return the sub-stream back to the host stream so that it can be reused
  // later. Sub-streams that are !ok() will not be reused.
  //
  // TODO(b/112196569): The semantics of failed sub-streams is error-prone.
  virtual void ReturnSubStream(Stream *sub_stream) TF_LOCKS_EXCLUDED(mu_);

  // Waits for another stream to complete.
  virtual tsl::Status WaitFor(Stream *other);

  // Waits for an event to be signaled.
  virtual tsl::Status WaitFor(Event *event);

  // Records an event at the tail of the stream.
  virtual tsl::Status RecordEvent(Event *event);
  ...
  virtual tsl::Status Memcpy(void *host_dst, const DeviceMemoryBase &gpu_src,
                             uint64_t size);
  ...
  // allocated by StreamExecutor::HostMemoryAllocate or otherwise allocated and
  // then registered with StreamExecutor::HostMemoryRegister.
  virtual tsl::Status Memcpy(DeviceMemoryBase *gpu_dst, const void *host_src,
                             uint64_t size);
  ...
  virtual tsl::Status Memcpy(DeviceMemoryBase *gpu_dst,
  ...
  virtual tsl::Status MemZero(DeviceMemoryBase *location, uint64_t size);
  ...
  virtual tsl::Status Memset32(DeviceMemoryBase *location, uint32_t pattern,
                               uint64_t size);
  ...
  virtual tsl::Status BlockHostUntilDone() TF_LOCKS_EXCLUDED(mu_);
  ...
  virtual tsl::Status DoHostCallbackWithStatus(
      absl::AnyInvocable<tsl::Status() &&> callback);
  Stream &ThenDoHostCallbackWithStatus(
      absl::AnyInvocable<tsl::Status() &&> callback);

  // Returns the StreamExecutor (parent object) associated with this stream.
  virtual StreamExecutor *parent() const {
    CHECK(parent_ != nullptr);
    return parent_;
  }

  //
  virtual CudaComputeCapability GetCudaComputeCapability() const {
    return parent()->GetDeviceDescription().cuda_compute_capability();
  }

  virtual RocmComputeCapability GetRocmComputeCapability() const {
